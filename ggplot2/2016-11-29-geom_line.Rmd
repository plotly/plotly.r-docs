...
fig <-  ggplot(data=dat1, aes(x=time, y=total_bill, group=sex, shape=sex)) +
    geom_line() +
    geom_point()
...
fig <-  ggplot(data=dat1, aes(x=time, y=total_bill, group=sex, shape=sex, colour=sex)) +
...
fig <-  ggplot(data=datn, aes(x=dose, y=length, group=supp, colour=supp)) +
    geom_line() +
    geom_point()
...
fig <-  ggplot(data=datn2, aes(x=dose, y=length, group=supp, colour=supp)) +
    geom_line() +
    geom_point()
...
fig <-  ggplot(d, aes(x=x, y=y))+geom_line()
#Change the length parameter for fewer or more points
thinned <- floor(seq(from=1,to=dim(d)[1],length=70))
fig <-  ggplot(d, aes(x=x, y=y))+geom_line()+geom_point(data=d[thinned,],aes(x=x,y=y))
...
fig <-  ggplot(data, aes(x = x, y= mean, group = as.factor(data$group),
...
fig <-  ggplot(d, aes(x=x, y=y, colour=treatment, group=interaction(treatment, replicate))) +
...
fig <-  ggplot(df, aes(x=x, y=y, linetype=fac)) +
    geom_line() +
