...
fig <-  ggplot(plotdata) + geom_line(aes(y=y, x=x, colour = "sin"))+
...
fig <-  ggplot(titanic3, aes(x = age, y = survived))
fig <-  fig + geom_point()
fig <-  fig + facet_grid(sex ~ pclass)
fig <-  fig + geom_line(data = sim.data, aes(y = mean))
fig <-  fig + geom_ribbon(data = sim.data, aes(y = mean, ymin = lo, ymax = hi),
                     alpha = .25)
fig <-  fig + labs(x = "Passenger Age", y = "Probability of Survival")
...
fig <-  ggplot(df,aes(x = score, y = pass)) +
    facet_wrap(~location) +
    geom_point(size=1) +
...
set.seed(101)
mfig <- data.frame(year=1990:2010)
N <- nrow(mp)
...
m01 <- gls(wow~poly(wav,3), data=mp, correlation = corARMA(fig=1))
...
fig <-  ggplot()+geom_ribbon(data=ribbon,aes(ymin=min,ymax=max,x=x.ribbon,fill='lightgreen'))+
