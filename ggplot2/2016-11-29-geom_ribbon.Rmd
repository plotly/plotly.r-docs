...
fig <-  p + geom_point()
fig <-  p + facet_grid(sex ~ pclass)
fig <-  p + geom_line(data = sim.data, aes(y = mean))
fig <-  p + geom_ribbon(data = sim.data, aes(y = mean, ymin = lo, ymax = hi),
                     alpha = .25)
fig <-  p + labs(x = "Passenger Age", y = "Probability of Survival")
