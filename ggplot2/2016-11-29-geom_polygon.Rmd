...
fig <-  ggplot(datapoly, aes(x=x, y=y)) + geom_polygon(aes(fill=value, group=id))
...
fig <-  qplot(data = df, x = x, y = y, colour = class) +
...
tmfig <-  with(mtcars, data.frame(x=c(0, 0, max(wt)*35), y=c(0, max(wt), max(wt))))

fig <-  ggplot(mtcars, aes(hp, wt)) +
...
fig <-  ggplot(df,aes(xmin=minX,xmax=maxX,ymin=minY,ymax=maxY,fill=Product))+
  geom_rect()
...
fig <-  ggplot(county_df, aes(long, lat, group = group)) +
...
#plot
fig <-  ggplot(US, aes(long, lat, group = group)) +
