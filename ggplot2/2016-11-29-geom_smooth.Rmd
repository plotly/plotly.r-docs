...
fig <-  qplot(speed, dist, data=cars)
fig <-  fig  geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
...
fig <-  ggplot(the.data, aes( year, value ) ) +
...
fig <-  ggplot(data = Batting_early_top10, aes(x = career.year, y = HR/AB)) +
  geom_point() +
