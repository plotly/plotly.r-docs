...
fig <-  ggplot(dfGamma, aes(x = values)) +
...
fig <-  ggplot(vegLengths, aes(length, fill = veg)) + geom_density(alpha = 0.2)
...
df <- data.frame(x <- rchisq(1000, 5, 10),
                 groufig <-  sample(LETTERS[1:5], size = 1000, replace = T))

fig <-  ggplot(df, aes(x, fill = group)) + 
