...
If the GeoJSON you are using either does not have an `id` field or you wish you use one of the keys in the `properties` field, you may use the `featureidkey` parameter to specify where to match the values of `id`.
In the following GeoJSON object/data-file pairing, the values of `properties.feature.id` match the values of the `feature.id` column:

```{r}
json_file$features$id[1]
counties$features.id[1]
```
