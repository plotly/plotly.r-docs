...
config(fig, displayModeBar = TRUE) %>%
  layout(
    plot_bgcolor = "#e5ecf6",
    xaxis = list(
      zerolinecolor = "#ffff",
...
config(fig, modeBarButtonsToRemove = c("zoom2d", "pan2d")) %>%
  layout(
    plot_bgcolor = "#e5ecf6",
    xaxis = list(
      zerolinecolor = "#ffff",
...
fig <- plot_ly(
  data = iris, x = ~Petal.Width, y = ~Sepal.Length, color = ~Species,
...
# Add modebar buttons
config(fig, modeBarButtonsToAdd = list(
  "drawline",
  "drawopenpath",
  "drawclosedpath",
  "drawcircle",
...
fig <- plot_ly() %>%
  add_trace(
    y = c(3, 5, 3, 2), x = c("2019-09-02", "2019-10-10", "2019-11-12", "2019-12-22"),
    type = "bar",
    texttemplate = "%{label}",
    textposition = "inside"
  ) %>%
...
config(fig, doubleClickDelay = 1000) %>%
  layout(
    plot_bgcolor = "#e5ecf6",
    xaxis = list(
      zerolinecolor = "#ffff",
